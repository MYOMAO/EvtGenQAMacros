...
void BRFinalAna(){
	...
	TString AntiHFName[NHFQA] = {"D^{-}","#bar D^{0}","D^{-}_{s}","#bar #Lambda_{c}^{-}","#bar B^{0}","B^{-}","#bar B^{0}_{s}","J/#psi","#Upsilon(1S)"};
	
	TString HFNameSave[NHFQA] = {"D+","D0","Ds","LambdaC","B0","B+","Bs","Jpsi","Upsilon"};
	TString AntiHFNameSave[NHFQA] = {"D-","D0Bar","DsBar","LambdaCBar","B0Bar","B-","BsBar","JpsiBar","UpsilonBar"};
	...
	TH1D * DiffAnti[NHFQA];
	TH1D * Diff2Anti[NHFQA];
	TH1D * Diff3Anti[NHFQA];
	...
	for(int i = 0; i < NHFQA; i++){
		...
		EvtGenAntiBR[i] = new TH1D(Form("EvtGenAntiBR_%d",i),"",NChannel,-0.5,NChannel-0.5);

		EvtGenAntiBR[i]->GetXaxis()->SetTitle(Form("EvtGen Simulation: %s Channel ID",AntiHFName[i].Data()));
		EvtGenAntiBR[i]->GetYaxis()->SetTitle("Branching Ratio");
		EvtGenAntiBR[i]->GetXaxis()->CenterTitle();
		EvtGenAntiBR[i]->GetYaxis()->CenterTitle();
		EvtGenAntiBR[i]->GetYaxis()->SetTitleOffset(0.7);
		EvtGenAntiBR[i]->GetYaxis()->SetTitleSize(0.06);

		EvtGenAntiBR[i]->SetMarkerStyle(20);
		EvtGenAntiBR[i]->SetMarkerSize(1);		
		EvtGenAntiBR[i]->SetLineColor(kRed);
		EvtGenAntiBR[i]->SetMarkerColor(kRed);

		EvtGenAntiBR[i]->SetMaximum(1.5);
		EvtGenAntiBR[i]->SetMinimum(5e-04);
		...
		for(int r = 0; r < NChannels; r++){
			...
			DiffAnti[i]->GetYaxis()->SetTitle("Dev From PDG");
			DiffAnti[i]->GetYaxis()->CenterTitle();
			DiffAnti[i]->GetXaxis()->SetLabelSize(0.20);
			DiffAnti[i]->GetXaxis()->SetLabelOffset(HFHadronStat->GetXaxis()->GetLabelOffset() * 6.2);

			Diff2Anti[i]->GetYaxis()->SetTitle("Dev From Decay Table");
			Diff2Anti[i]->GetYaxis()->CenterTitle();
			Diff2Anti[i]->GetXaxis()->SetLabelSize(0.20);
			Diff2Anti[i]->GetXaxis()->SetLabelOffset(HFHadronStat->GetXaxis()->GetLabelOffset() * 6.2);


			Diff3Anti[i]->GetYaxis()->SetTitle("Dev From PDG");
			Diff3Anti[i]->GetYaxis()->CenterTitle();
			Diff3Anti[i]->GetXaxis()->SetLabelSize(0.20);
			Diff3Anti[i]->GetXaxis()->SetLabelOffset(HFHadronStat->GetXaxis()->GetLabelOffset() * 6.2);
		...
		DiffAnti[i] = new TH1D(Form("Diff_%d",i),"",NChannel,-0.5,NChannel-0.5);
		//Diff[i]->GetXaxis()->SetTitle(Form("%s Channel ID",HFName[i].Data()));
		DiffAnti[i]->GetYaxis()->SetTitle("Dev From PDG");
		DiffAnti[i]->GetXaxis()->CenterTitle();
		DiffAnti[i]->GetYaxis()->CenterTitle();
		DiffAnti[i]->GetYaxis()->SetTitleOffset(0.4);
		DiffAnti[i]->GetYaxis()->SetTitleSize(0.10);

		DiffAnti[i]->GetXaxis()->SetTitleOffset(1.0);
		DiffAnti[i]->GetXaxis()->SetTitleSize(0.13);


		DiffAnti[i]->SetMarkerStyle(20);
		DiffAnti[i]->SetMarkerSize(1);		
		DiffAnti[i]->SetLineColor(kBlack);
		DiffAnti[i]->SetMarkerColor(kBlack);
		DiffAnti[i]->GetXaxis()->SetLabelSize(0.15);
		DiffAnti[i]->GetYaxis()->SetLabelSize(0.07);

		DiffAnti[i]->SetMaximum(1.6);
		DiffAnti[i]->SetMinimum(-1.6);
...
*/
		...
		AntiBR1DHis[i] = (TH1D *) fin->Get(Form("AntiBR1DHis_%d",i));
		TotalStat = HFAntiHadronStat->GetBinContent(i+1);
		...
		c->SaveAs(Form("Plots/%s/EvtGenPDG.png",HFNameSave[i].Data()));
			...
			ChannelStat = AntiBR1DHis[i]->GetBinContent(j+1);
			ChannelStatErr = AntiBR1DHis[i]->GetBinError(j+1);
			BR = ChannelStat/TotalStat;
			BRErr = ChannelStatErr/TotalStat;

			EvtGenAntiBR[i]->SetBinContent(j+1,BR);
			EvtGenAntiBR[i]->SetBinError(j+1,BRErr);

			Dev =  (BR - PDGValue[i][j])/PDGValue[i][j];
			DevErr = (BR/PDGValue[i][j]) * sqrt(BRErr/BR * BRErr/BR + PDGValueErr[i][j]/PDGValue[i][j] * PDGValueErr[i][j]/PDGValue[i][j]);

			DiffAnti[i]->SetBinContent(j+1,Dev);
			DiffAnti[i]->SetBinError(j+1,DevErr);
			...
			Diff2Anti[i]->SetBinContent(j+1,Dev2);
			Diff2Anti[i]->SetBinError(j+1,Dev2Err);
			...
			Diff3Anti[i]->SetBinContent(j+1,Dev3);
			Diff3Anti[i]->SetBinError(j+1,Dev3Err);
		...
		leg5[i] = new TLegend(0.12,0.60,0.50,0.75,NULL,"brNDC");
		leg5[i]->SetBorderSize(0);
		leg5[i]->SetTextSize(0.045);
		leg5[i]->SetTextFont(42);
		leg5[i]->SetFillStyle(0);
		leg5[i]->SetLineWidth(2);
		...
		EvtGenAntiBR[i]->Draw("ep");
		PDGBR[i]->Draw("epSAME");

		lat->DrawLatex(0.15,0.78,AntiHFName[i].Data());
		lat->SetTextSize(0.05);	
		lat->DrawLatex(0.40,0.78,Form("Total Statistics: %d",TotalEvents[i]));
		lat->SetTextSize(0.1);	

		leg5[i]->AddEntry(EvtGenBR[i],"EvtGen","PL");
		leg5[i]->AddEntry(PDGBR[i],"PDG 2022","PL");

		leg5[i]->Draw("SAME");

		c->cd();

		TPad* p5 = new TPad("p2","p2",0,0,1,0.3);
		p5->SetTopMargin(0);
		...
		c->SaveAs(Form("Plots/%s/SimPDG.png",AntiHFNameSave[i].Data()));



		c->cd();

		leg6[i] = new TLegend(0.12,0.60,0.50,0.75,NULL,"brNDC");
		leg6[i]->SetBorderSize(0);
		leg6[i]->SetTextSize(0.045);
		leg6[i]->SetTextFont(42);
		leg6[i]->SetFillStyle(0);
		leg6[i]->SetLineWidth(2);

		TPad* p6 = new TPad("p1","p1",0,0.3,1,1);
		p6->SetBottomMargin(0);
		...
		leg6[i]->AddEntry(EvtGenBR[i],"EvtGen Sim","PL");
		leg6[i]->AddEntry(EvtGenEncoded[i],"EvtGen Decay Table","PL");

		leg6[i]->Draw("SAME");

		c->cd();

		TPad* p7 = new TPad("p2","p2",0,0,1,0.3);
		p7->SetTopMargin(0);
		p7->SetBottomMargin(0.3);

		p7->Draw();PDGcTau[i] * Light * unit;
		...
		c->SaveAs(Form("Plots/%s/SimEvtGen.png",AntiHFNameSave[i].Data()));
		...
		leg7[i] = new TLegend(0.12,0.60,0.50,0.75,NULL,"brNDC");
		leg7[i]->SetBorderSize(0);
		leg7[i]->SetTextSize(0.045);
		leg7[i]->SetTextFont(42);
		leg7[i]->SetFillStyle(0);
		leg7[i]->SetLineWidth(2);

		TPad* p8 = new TPad("p1","p1",0,0.3,1,1);
		p8->SetBottomMargin(0);
		...
		leg7[i]->AddEntry(EvtGenEncoded[i],"EvtGen Decay Table","PL");		
		leg7[i]->AddEntry(PDGBR[i],"PDG 2022","PL");

		leg7[i]->Draw("SAME");

		c->cd();

		TPad* p8 = new TPad("p2","p2",0,0,1,0.3);
		p8->SetTopMargin(0);
		...
		c->cd();
		c->SaveAs(Form("Plots/%s/EvtGenPDG.png",AntiHFNameSave[i].Data()));
